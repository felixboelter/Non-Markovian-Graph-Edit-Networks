...
class EvolveGCNO(torch.nn.Module):
    ...
    def forward(
        self,
        X: torch.FloatTensor,
        adjacency_matrix: torch.LongTensor,
        ...
        self.weight = torch.nn.Parameter(W.squeeze(0))
        # Convert the graph from an adjacency matrix to a sparse graph
        ...
        # Output going through the linear layer
        X = self.linear(X) 
        return X
...
class EvolveGCNH(torch.nn.Module):
    ...
    Args:
        ...
        add_self_loops (bool, optional): If set to :obj:`False`, will not add
            self-loops to the input graph. (default: :obj:`True`)
    """
    def __init__(self, num_of_nodes: int, in_channels: int, out_channels: int, num_layers: int, improved: bool=False,
    ...
    def _create_layers(self):
        ...
        self.conv_layer = GCNConv(in_channels = self.in_channels,
                                  ...
                                  bias = False)
        self.linear = torch.nn.Linear(self.in_channels, self.out_channels)
    def forward(self, X: torch.FloatTensor, adjacency_matrix: torch.LongTensor, 
...
class GEN(torch.nn.Module):
    ...
    """
    def __init__(self, num_layers, dim_in, dim_hid, nonlin = torch.nn.ReLU(), filter_edge_edits = False, non_markovian = True):
        ...
        if non_markovian == True:
            # first, an input layer
            self._layers.append(EvolveGCNO(self._dim_in, self._dim_hid[0]))
            # # then, all hidden layers
            for l in range(1, self._num_layers):
                self._layers.append(EvolveGCNO(self._dim_hid[l-1], self._dim_hid[l]))
        elif non_markovian == False:
            # first, an input layer
    ...
    def forward(self, A, X = None):
        ...
        if X is None:
            X = torch.zeros(A.shape[0], self._dim_in,dtype=torch.float) # changed
        ...
        for l in range(self._num_layers):
            X = self._nonlin(self._layers[l](X.to(device), A.to(device)))
...
class GEN_loss_crossent(torch.nn.Module):
    ...
    of tuples (i, j) where i is a node such that no outgoing edge is edited
