...
def teaching_protocol(year, month, past = 12, future = 12):
    ...
    A, I = read_graph_from_csv(os.path.join(os.path.abspath,'hep-th/graphs/%s.csv' % past_months[0]))
    for tau in range(1, len(past_months)):
        # parse the current graph from CSV data
        A2, I2 = read_graph_from_csv(os.path.join(os.path.abspath,'hep-th/graphs/%s.csv' % past_months[tau]))
        # add A2, I2 to A, I
    ...
    B, J = read_graph_from_csv(os.path.join(os.path.abspath,'hep-th/graphs/%s.csv' % _month_strings[t]))
    for tau in range(1, len(future_months)):
        # parse the current graph from CSV data
        B2, J2 = read_graph_from_csv(os.path.join(os.path.abspath,'hep-th/graphs/%s.csv' % future_months[tau]))
        # add B2, J2 to B, B
...
def compute_loss(gen, year, month, past = 12):
    ...
    if t < past:
        ...
        past_months = _month_strings[(t-past):t]
    # Read current graph
    A, I = read_graph_from_csv(os.path.join(os.path.dirname(__file__),'hep-th/graphs/%s.csv' % past_months[0]))
    for tau in range(1, len(past_months)-1):
        # Update the weights of the EvolveGCN
        # _, _ = gen(torch.tensor(A, dtype=torch.float))
        # parse the current graph from CSV data
        A , I = read_graph_from_csv(os.path.join(os.path.dirname(__file__),'hep-th/graphs/%s.csv' % past_months[tau]))
    # parse the next graph from CSV data
    B, J = read_graph_from_csv(os.path.join(os.path.dirname(__file__),'hep-th/graphs/%s.csv' % _month_strings[t]))
